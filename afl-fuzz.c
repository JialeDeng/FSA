...
EXP_ST u32 num_syntax_err = 0;        /* num of syntax error              */
...
struct queue_entry {
  ...
  u32 len;                            /* Input length--testcase size      */
...
retry_external_pick:
  ...
  do { 
    ...
    /* UR: Generate a random number */
  } while (tid == current_entry && queued_paths > 1);

  target = queue;

  // get the tid^th testcase
  while (tid >= 100) { 
  ...
  /* queue_cur: Current offset within the queue  */
  while ( target && 
          calculate_score(target) >= 100 && /* select target with better score */
          (target->len < 2 || target == queue_cur) && 
          queued_paths > 1) {
    target = target->next;
    splicing_with++;
  }
